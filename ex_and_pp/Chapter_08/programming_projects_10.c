...
int main(void)
{
	int time_table[2][NUMBER] = {{480, 583, 679, 767, 840, 945, 1140, 1305},	// row 1: departure
					   			 {616, 712, 811, 900, 968, 1075, 1280, 1438}};	// row 2: arrival
	int hour, minute, convert, compare, n, close, arrival, ap;
	char check_ap;
	...
	close = 2000; // to put the first departure time.

	for (int i = 0; i < 8; i++) {
		compare = abs(time_table[0][i] - convert);
		if (close > compare) {
			close = compare;
			n = i;
		}
	...
	close = time_table[0][n];
	arrival = time_table[1][n];
